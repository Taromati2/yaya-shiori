...
bool Ccct::MbcsToUcs2Buf(aya::string_t &out, const char *mstr, int charset)
{
	if ( charset == CHARSET_UTF8 ) {
		out.erase();
		...
		aya::char_t *p = mbcs_to_utf16be(mstr,charset);
		if ( p ) {
			out = p;
			free(p);
			return true;
...
bool Ccct::MbcsToUcs2Buf(aya::string_t &out, const std::string &mstr, int charset)
{
