 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::DICUNLOAD(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::UNDEFFUNC(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t funcname = arg.array()[0].s_value;
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::UNDEFGLOBALDEFINE(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t defname = arg.array()[0].s_value;
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::ISGLOBALDEFINE(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::APPEND_RUNTIME_DIC(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t def = arg.array()[0].s_value;
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::SETGLOBALDEFINE(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t defname = arg.array()[0].s_value;
	aya::string_t defbody = arg.array()[1].GetValueString();
 ...
 * -----------------------------------------------------------------------
 */
CValue CSystemFunction::GETFUNCINFO(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t name = arg.array()[0].GetValueString();
 ...
 * -----------------------------------------------------------------------
 */
CValue CSystemFunction::PROCESSGLOBALDEFINE(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
	...
	aya::string_t aret = arg.array()[0].GetValueString();
...
CValue	CSystemFunction::ASORT(const CValue &arg, aya::string_t &d, int &l)
{
	...
	if(sz <= 2) {
		...
		if( option.find(L"index") != aya::string_t::npos ) {
			...
			rval.array().emplace_back(arg.array()[1]);
		}
		return rval;
	}
	...
	if( option.find(L"index") != aya::string_t::npos ) {
		...
		unsigned int n = sort_vector.size();
		for ( unsigned int i = 0 ; i < n ; ++i ) {
			rval.array().emplace_back(arg.array()[sort_vector[i]]);
		}
	}
...
CValue	CSystemFunction::SPLIT(const CValue &arg, aya::string_t &d, int &l)
{
	...
	if(nums == 1 || sep_str.length() == 0) {
		result.array().emplace_back(CValueSub(arg.array()[0].GetValueString()));
		return result;
	}
 ...
 * -----------------------------------------------------------------------
 */
CValue	CSystemFunction::SETTAMAHWND(const CValue &arg, aya::string_t &d, int &l)
{
	if(!arg.array_size()) {
...
CValue	CSystemFunction::GETSYSTEMFUNCLIST(const CValue &arg, aya::string_t &/*d*/, int &/*l*/)
{
	...
	if( name.empty() ) {
		for(auto i:sysfunc) {
			if(i[0])
				result.array().emplace_back(CValueSub(i));
		}
	}
	//ある場合
	else {
