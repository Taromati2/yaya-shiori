...
inline CFunction::CFunction(CAyaVM& vmr, const aya::string_t& n, choicetype_t ct, const aya::string_t& df, int lc) : pvm(&vmr), name(n), dupl_func(ct), dicfilename(df), linecount(lc), dicfilename_fullpath(vmr.basis().ToFullPath(df))
{
	statelenm1 = 0;
	namelen = name.size();
}
 ...
 * -----------------------------------------------------------------------
 */
size_t	CFunction::ExecuteInBrace(size_t line, CValue &result, CLocalVariable &lvar, aya::int_t type, int &exitcode, std::vector<CVecValue>* UpperLvCandidatePool,bool inpool)
{
	// 開始時の処理
	lvar.AddDepth();
...
void	CFunction::ExecHistoryP2(CCell& cell, CStatement &st)
{
	...
	aya::int_t start = std::min<aya::int_t>(st.cell().size()-1,cell.ansv_const().GetValueInt());

	for(aya::int_t i = start ; i >= 0; i--) {
