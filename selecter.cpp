...
choicetype_t CSelecter::StringToChoiceType(const aya::string_t& ctypestr, CAyaVM &vm, const aya::string_t& dicfilename, size_t linecount)
{
	...
	else if ( checkstr.find(L"all") != aya::string_t::npos ) {
		outtype = CHOICETYPE_ALL_FLAG;
		aya::ws_replace(checkstr,L"all",L"");
	}
	else if ( checkstr.find(L"last") != aya::string_t::npos ) {
		outtype = CHOICETYPE_LAST_FLAG;
		aya::ws_replace(checkstr,L"last",L"");
	}
 ...
 * -----------------------------------------------------------------------
 */
aya::string_t CSelecter::ChoiceTypeToString(choicetype_t ctype)
{
	aya::string_t aret;
