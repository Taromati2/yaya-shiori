...
void	CLogExCode::StructCellString(std::vector<CCell> *cellvector, aya::string_t &formula)
{
	...
	for(std::vector<CCell>::iterator it = cellvector->begin(); it != cellvector->end(); it++) {
		...
		case F_TAG_ARRAYORDER:
			...
			break;
		case F_TAG_INT:
			tmpstr = L"(int)" + aya::ws_lltoa(it->value_const().i_value) + L' ';
			formula += tmpstr;
			break;
		case F_TAG_DOUBLE:
...
void	CLogExCode::OutVariableInfoForCheck(void)
{
	...
	for(size_t	i = 0; i < var_num; i++) {
		...
		case F_TAG_INT:
			tmpstr = L"(int)" + aya::ws_lltoa(var->value_const().i_value) + L"\n";
			vm.logger().Write(tmpstr);
			break;
		case F_TAG_DOUBLE:
...
void	CLogExCode::StructArrayString(const CValueArray &vs, aya::string_t &enlist)
{
	...
	for(CValueArray::const_iterator it = vs.begin(); it != vs.end(); it++) {
		switch(it->GetType()) {
		case F_TAG_INT:
			tmpstr = L"(int)" + aya::ws_lltoa(it->GetValueInt()) + L' ';
			enlist += tmpstr;
			break;
		case F_TAG_DOUBLE:
			tmpstr = L"(double)" + aya::ws_ftoa(it->GetValueDouble()) + L' ';
			enlist += tmpstr;
			break;
		case F_TAG_STRING:
			enlist += L"(string)";
