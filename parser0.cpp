 ...
 * -----------------------------------------------------------------------
 */
int CParser0::DynamicAppendRuntimeDictionary(const aya::string_t& codes)
{
	vm.func_parse_new();
 ...
 * -----------------------------------------------------------------------
 */
int	CParser0::DynamicUndefFunc(const aya::string_t& funcname)
{
	vm.func_parse_new();
...
char	CParser0::StoreInternalStatement(int targetfunc, aya::string_t &str, int& depth, const aya::string_t& dicfilename, int linecount)
{
	...
	if (!str.compare(L"{")) {
		depth++;
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_OPEN, linecount));
		return 1;
	}
	// }
	else if (!str.compare(L"}")) {
		depth--;
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_CLOSE, linecount));
		return 1;
	}
	// others　elseへ書き換えてしまう
	else if (!str.compare(L"others")) {
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_ELSE, linecount));
		return 1;
	}
	// else
	...
	// break
		...
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_BREAK, linecount));
		return 1;
	}
	// continue
	else if (!str.compare(L"continue")) {
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_CONTINUE, linecount));
		return 1;
	}
	// return
	else if (!str.compare(L"return")) {
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_RETURN, linecount));
		return 1;
	}
	// --
	else if (!str.compare(L"--")) {
		vm.function_parse().func[targetfunc].statement.emplace_back(CStatement(ST_COMBINE, linecount));
		return 1;
	}
