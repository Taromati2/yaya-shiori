 ...
 * -----------------------------------------------------------------------
 */
int	CParser0::DynamicUnloadDictionary(aya::string_t dicfilename)
{
...
*/
	...
	if (IsIntString(scell.value_const().s_value)) {
		scell.value() = aya::ws_atoll(scell.value_const().s_value);
		return 0;
	}
	// 整数リテラル(BIN)
	if (IsIntBinString(scell.value_const().s_value, 1)) {
		scell.value() = aya::ws_atoll(scell.value_const().s_value, 2);
		return 0;
	}
	// 整数リテラル(HEX)
	if (IsIntHexString(scell.value_const().s_value, 1)) {
		scell.value() = aya::ws_atoll(scell.value_const().s_value, 16);
		return 0;
	}
	// 実数リテラル
...
inline CDefine::CDefine(CAyaVM& vm, const aya::string_t& bef, const aya::string_t& aft, const aya::string_t& df):
	...
	if(is_macro_function){
		...
		aya::string_t args = regex.GetMatch(before.c_str(), 2);
		CRegexpT<aya::char_t> regex2(L"([^!\"#$%&'()*+\\-/:;<=>?@\\[\\\\\\]`{|}\\s]+)", SINGLELINE);
