...
public:
	CFunction(CAyaVM &vmr, const aya::string_t& n, choicetype_t ct, const aya::string_t& df, int lc) : pvm(&vmr) , name(n) , dupl_func(ct) , dicfilename(df) , linecount(lc)
	{
		statelenm1  = 0;
		namelen     = name.size();
	}
...
protected:
	size_t	ExecuteInBrace(size_t line, CValue& result, CLocalVariable& lvar, aya::int_t type, int& exitcode, std::vector<CVecValue>* UpperLvCandidatePool, bool inpool);
