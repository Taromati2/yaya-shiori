...
public:
	CStatement(int t, ptrdiff_t l, std_shared_ptr<CDuplEvInfo> dupl = NULL)
	{
		type = t;
		linecount = l;
		jumpto = 0;
...
public:
	CFunction(CAyaVM& vmr, const aya::string_t& n, choicetype_t ct, const aya::string_t& df, int lc);
...
protected:
	...
	ExecutionInBraceResult	ExecuteInBrace(size_t line, CLocalVariable& lvar, aya::int_t type, int& exitcode, std::vector<CVecValue>* UpperLvCandidatePool, bool inpool);
...
public:
	...
	ptrdiff_t GetFunctionIndexFromName(const aya::string_t& name);
	void AddFunctionIndex(const aya::string_t& name,size_t index);
